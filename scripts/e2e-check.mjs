...
step('travel mode: gym profile + auto-adapted routine')
await page.locator('.bottom-nav').getByRole('link', { name: 'Workout' }).click()
await page.getByText('Start empty workout').waitFor()
console.log('gym chip:', (await page.locator('.gym-chip').textContent()).trim())
await page.locator('.gym-chip').click()
await page.getByRole('button', { name: '+ Add a gym' }).click()
await page.getByPlaceholder('e.g. Hotel gym').fill('Hotel')
await page.locator('label', { hasText: 'Bench' }).locator('input[type=checkbox]').check()
await page.getByRole('button', { name: 'Save & use this gym' }).click()
await page.locator('.gym-chip', { hasText: 'Hotel' }).waitFor()
console.log('Hotel gym (dumbbells + bench) active ✔')
await page.locator('.list-item', { hasText: 'Push Day' }).click()
await page.getByText(/sets done/).waitFor()
await page.locator('.card', { hasText: 'Dumbbell Bench Press' }).getByText('swapped from Bench Press').waitFor()
await page.locator('.card', { hasText: 'Dumbbell Shoulder Press' }).getByText('swapped from Overhead Press').waitFor()
const swapChips = await page.locator('.suggest-swap').count()
console.log(`auto-substituted with chips (expect 2): ${swapChips}; Lateral Raise untouched: ${await page.locator('.card', { hasText: 'Lateral Raise' }).count()}`)
await shot('19-travel-adapted')

step('travel mode: manual swap + quick workout')
await page.locator('.card', { hasText: 'Lateral Raise' }).getByRole('button', { name: 'Swap exercise' }).click()
await page.locator('.modal .list-item').first().waitFor()
const firstAlt = (await page.locator('.modal .list-item').first().locator('.grow > div').first().textContent()).trim()
await page.locator('.modal .list-item').first().click()
await page.locator('.card', { hasText: firstAlt }).getByText('swapped from Lateral Raise').waitFor()
console.log(`manual swap: Lateral Raise → ${firstAlt} ✔`)
await page.getByRole('button', { name: 'Discard workout' }).click()
await page.getByRole('button', { name: /Quick workout for Hotel/ }).click()
await page.getByText(/Quick workout · Hotel/).waitFor()
const quickNames = await page.locator('.card a').allTextContents()
const banned = quickNames.filter((n) => /Barbell|Cable|Machine|Pulldown|Back Squat|Deadlift|Overhead Press/.test(n))
console.log(`quick workout entries (${quickNames.length}):`, quickNames.join(' | '))
console.log(banned.length === 0 ? 'all entries fit Hotel equipment ✔' : `EQUIPMENT VIOLATIONS: ${banned}`)
await shot('20-quick-workout')
await page.getByRole('button', { name: 'Discard workout' }).click()
await page.getByText('Start empty workout').waitFor()
// back to full gym → no adaptation
await page.locator('.gym-chip').click()
await page.locator('.modal .list-item', { hasText: 'Full gym' }).click()
await page.locator('.gym-chip', { hasText: 'Full gym' }).waitFor()
await page.locator('.list-item', { hasText: 'Push Day' }).click()
await page.getByText(/sets done/).waitFor()
const benchBack = await page.locator('.card a', { hasText: /^Bench Press$/ }).count()
const chipsAtFullGym = await page.locator('.suggest-swap').count()
console.log(`full gym restore: Bench Press present ${benchBack}, swap chips ${chipsAtFullGym} (expect 1, 0)`)
await page.getByRole('button', { name: 'Discard workout' }).click()
await page.getByText('Start empty workout').waitFor()
...
// online-only actions announce themselves instead of failing
await page.getByRole('link', { name: /Exercises/ }).click()
await page.getByPlaceholder('Search exercises…').fill('curl')
const wgerBtn = page.getByRole('button', { name: /Online search unavailable offline/ })
console.log('wger search disabled offline:', await wgerBtn.isDisabled())
// core logging still works with zero network: log food + record a workout set
await page.locator('.bottom-nav').getByRole('link', { name: 'Food' }).click()
await page.locator('.card', { hasText: 'Snacks' }).getByRole('button', { name: '+ Add food' }).click()
console.log('barcode scan disabled offline:', await page.getByRole('button', { name: 'Scan' }).isDisabled())
await page.getByRole('button', { name: 'Quick add', exact: true }).click()
await page.locator('label:has-text("Calories (kcal)") input').fill('120')
await page.getByRole('button', { name: 'Add to diary' }).click()
await page.locator('.card', { hasText: 'Snacks' }).getByText('120 kcal').first().waitFor()
console.log('offline food logging → IndexedDB ✔')
await page.locator('.bottom-nav').getByRole('link', { name: 'Workout' }).click()
await page.getByText('Start empty workout').click()
await page.getByText(/sets done/).waitFor()
await page.getByRole('button', { name: '+ Add exercise' }).click()
await page.getByPlaceholder('Search exercises…').fill('push-up')
await page.locator('.modal .list-item', { hasText: 'Push-Up' }).first().click()
await page.locator('.set-grid input').nth(1).fill('20')
await page.locator('.set-done-btn').first().click()
await page.locator('.rest-timer').waitFor()
console.log('offline workout logging + rest timer ✔')
await page.getByRole('button', { name: 'Skip' }).click()
await page.getByRole('button', { name: 'Discard workout' }).click()
await page.getByText('Start empty workout').waitFor()
await ctx.setOffline(false)
await page.locator('.offline-banner').waitFor({ state: 'hidden' })
console.log('banner clears when back online ✔')
